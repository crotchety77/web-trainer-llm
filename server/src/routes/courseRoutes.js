...
const ATTACHMENTS_DIR = path.resolve(process.cwd(), "uploads", "lecture-attachments");
...
function validateLectureAttachment(file) {
  if (!file) {
...
function buildAttachmentDownloadUrl(storedName) {
...
function normalizeBlockPayload(body) {
  ...
  return {
    ...
    attachmentUrl: type === "lecture" ? String(body.attachment_url || "").trim() : "",
    position: Number(body.position) || 1,
